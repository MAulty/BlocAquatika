...
public class Wooden_Speargun extends ShootableItem implements IVanishable {
    /** Set to {@code true} when the speargun is 20% charged. */
    private boolean isLoadingStart = false;
    /** Set to {@code true} when the speargun is 50% charged. */
    private boolean isLoadingMiddle = true;

    public Wooden_Speargun(Item.Properties propertiesIn) {
        super(propertiesIn);
    }
    ...
    private static void addChargedProjectile(ItemStack wooden_speargun, ItemStack projectile) {
        CompoundNBT compoundnbt = wooden_speargun.getOrCreateTag();
        ListNBT listnbt;
    ...
    private static void fireProjectile(World worldIn, LivingEntity shooter, Hand handIn, ItemStack wooden_speargun, ItemStack projectile, float soundPitch, boolean isCreativeMode, float velocity, float inaccuracy, float projectileAngle) {
        if (!worldIn.isRemote) {
            ...
            if (flag) {
                ...
                projectileentity = createArrow(worldIn, shooter, wooden_speargun, projectile);
            ...
            if (shooter instanceof ICrossbowUser) {
                ...
                icrossbowuser.func_230284_a_(icrossbowuser.getAttackTarget(), wooden_speargun, projectileentity, projectileAngle);
            } else {
            ...
            wooden_speargun.damageItem(flag ? 3 : 1, shooter, (p_220017_1_) -> {
